...
aeqd provides full support for anonymous networking via the
...
[stream isolation](#TorStreamIsolation).  In addition, aeqd supports a hybrid,
...
Configuring aeqd as a Tor client is straightforward.  The first step is
obviously to install Tor and ensure it is working. Once that is done, all that
typically needs to be done is to specify the `--proxy` flag via the aeqd command
line or in the aeqd configuration file.  Typically the Tor proxy address will be
...
By default, aeqd assumes the proxy specified with `--proxy` is a Tor proxy and
...
discuss since aeqd will act as the server.
...
```bash
$ ./aeqd --proxy=127.0.0.1:9050 --listen=127.0.0.1 --externalip=fooanon.onion
```
...
aeqd provides support for operating as a bridge between regular nodes and hidden
...
by using the `--onion` flag via the aeqd command line or in the aeqd
...
```bash
$ ./aeqd --onion=127.0.0.1:9050 --externalip=fooanon.onion
```
...
aeqd provides support for Tor stream isolation by using the `--torisolation`
...
```bash
$ ./aeqd --proxy=127.0.0.1:9050 --torisolation
```
