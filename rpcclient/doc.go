...
style Decred JSON-RPC API.  This client has been tested with aeqd
(https://github.com/decred/aeqd) and dcrwallet
(https://github.com/decred/dcrwallet).

In addition to the compatible standard HTTP POST JSON-RPC API, aeqd and
...
aeqd or dcrwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by aeqd and
...
All notifications provided by aeqd require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by aeqd (and
...
Also, it is important to realize that aeqd intentionally separates the wallet
functionality into a separate process named dcrwallet.  This means if you are
connected to the aeqd RPC server directly, only the RPCs which are related to
