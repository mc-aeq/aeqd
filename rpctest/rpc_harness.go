...
// Harness fully encapsulates an active aeqd process to provide a unified
// platform for creating rpc driven integration tests involving aeqd. The
// active aeqd node will typically be run in simnet mode in order to allow for
// easy generation of test blockchains.  The active aeqd process is fully
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	// Start the aeqd node itself. This spawns a new process which will be
	// managed
	...
	// Ensure aeqd properly dispatches our registered call-back for each new
...
// connectRPCClient attempts to establish an RPC connection to the created aeqd
