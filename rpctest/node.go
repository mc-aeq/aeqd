...
// nodeConfig contains all the args, and data required to launch a aeqd process
...
// arguments returns an array of arguments that be used to launch the aeqd
// process.
...
// command returns the exec.Cmd which will be used to start the aeqd process.
...
// to the aeqd process that is launched via Start().
...
// as the base for the log and data directories for aeqd.
...
// start creates a new aeqd process, and writes its pid in a file reserved for
...
func (n *node) start() error {
	...
	pid, err := os.Create(filepath.Join(n.config.String(), "aeqd.pid"))
	if err != nil {
		return err
	}
...
// stop interrupts the running aeqd process process, and waits until it exits
...
// shutdown terminates the running aeqd process, and cleans up all
