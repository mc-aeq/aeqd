...
func warnMultipleDBs() {
	...
	if len(duplicateDbPaths) > 0 {
		...
		aeqdLog.Warnf("WARNING: There are multiple block chain databases "+
...
func loadBlockDB() (database.DB, error) {
	...
	if cfg.DbType == "memdb" {
		aeqdLog.Infof("Creating block database in memory.")
	...
	aeqdLog.Infof("Loading block database from '%s'", dbPath)
	...
	aeqdLog.Info("Block database loaded")
	return db, nil
}
