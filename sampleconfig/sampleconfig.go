...
// FileContents is a string containing the commented example config for aeqd.
...
; The default is ~/.aeqd/data on POSIX OSes, $LOCALAPPDATA/aeqd/data on Windows,
; ~/Library/Application Support/aeqd/data on macOS, and $homed/aeqd/data on
; Plan9.  Environment variables are expanded so they may be used.  NOTE: Windows
; environment variables are typically %VARIABLE%, but they must be accessed with
; $VARIABLE here.
; datadir=~/.aeqd/data                            ; Unix
; datadir=$LOCALAPPDATA/aeqd/data                 ; Windows
; datadir=~/Library/Application Support/aeqd/data ; macOS
...
; line.  aeqd will not contact 3rd-party sites to obtain external ip addresses.
...
; with, but the behavior is slightly different.  By default, aeqd will query DNS
...
; Disable DNS seeding for peers.  By default, when aeqd starts, it will use
...
; which is used to control and query information from a running aeqd process.
;
...
; log level for individual subsystems.  Use aeqd --debuglevel=show to list
