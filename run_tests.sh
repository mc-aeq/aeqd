...
GOVERSION=${1:-1.10}
REPO=aeqd
DOCKER_IMAGE_TAG=mc-aeq-golang-builder-$GOVERSION
...
if [ -f ~/.cache/$DOCKER_IMAGE_TAG.tar ]; then
	...
	docker pull mc-aeq/$DOCKER_IMAGE_TAG
	if [ $? != 0 ]; then
		echo 'docker pull failed'
		exit 1
	fi
	docker save mc-aeq/$DOCKER_IMAGE_TAG > ~/.cache/$DOCKER_IMAGE_TAG.tar
	if [ $? != 0 ]; then
...
docker run --rm -it -v $(pwd):/src mc-aeq/$DOCKER_IMAGE_TAG /bin/bash -c "\
