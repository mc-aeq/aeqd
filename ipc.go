...
// processes to communicate with and and manage the lifetime of a aeqd child
...
// message, shutdown begins.  This prevents aeqd from continuing to run
...
func serviceControlPipeRx(fd uintptr) {
	...
	for {
		...
		if err != nil {
			aeqdLog.Errorf("Failed to read from pipe: %v", err)
			break
		}
	}
...
// for a child aeqd process to communicate with a parent process without the
...
func serviceControlPipeTx(fd uintptr) {
	...
	aeqdLog.Errorf("Failed to write to pipe: %v", err)
}
